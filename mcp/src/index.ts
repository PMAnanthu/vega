...
server.tool(
  "vega_get_task",
  "Get full details of a task by ID or title keyword.",
  {
  ...
  async ({ id, keyword }) => {
    ...
    if (id) {
      ...
      const q = keyword.toLowerCase();
      const matches = tasks.filter((t: any) => (t.title || "").toLowerCase().includes(q));
      if (matches.length === 0) throw new Error(`No task matching: ${keyword}`);
      if (matches.length > 1) return { content: [{ type: "text", text: `Multiple matches (${matches.length}) — use id:\n${JSON.stringify(matches.map((t: any) => ({ id: t.id, title: t.title, status: t.status })), null, 2)}` }] };
      task = matches[0];
    } else {
...
server.tool(
  "vega_create_task",
  "Create a new task (or sub-task) in Vega.",
  {
...
server.tool(
  ...
  async ({ id, ...updates }) => {
    ...
    if (updates.status === "completed") { upd.completedAt = now(); upd.completed = true; }
...
server.tool(
  "vega_delete_task",
  "Delete a task (and optionally its sub-tasks) by ID.",
  {
  ...
  async ({ id, deleteSubtasks }) => {
    ...
    if (deleteSubtasks) {
      ...
      const addChildren = (pid: string) => {
        tasks.filter((t: any) => t.parentId === pid).forEach((t: any) => { toDelete.add(t.id); addChildren(t.id); });
      };
      addChildren(id);
    }
    const filtered = tasks.filter((t: any) => !toDelete.has(t.id));
...
server.tool(
  "vega_list_subtasks",
  "List all direct sub-tasks of a parent task.",
  { parentId: z.string().describe("Parent task ID") },
  async ({ parentId }) => {
    const tasks = await vegaGet("/api/tasks") as any[];
    const subtasks = tasks.filter((t: any) => t.parentId === parentId);
...
server.tool(
  "vega_list_idea_docs",
  "List all documents (markdown, canvas, diagram, calendar) inside an idea.",
  { ideaId: z.string() },
  async ({ ideaId }) => {
    const ideas = await vegaGet("/api/ideas") as any[];
    const idea = ideas.find((i: any) => i.id === ideaId);
    if (!idea) throw new Error(`Idea not found: ${ideaId}`);
    const docs = (idea.docs || []).map((d: any) => ({
...
server.tool(
  "vega_get_idea_doc",
  "Get the full content of a document inside an idea.",
  {
    ideaId:       z.string(),
  ...
  async ({ ideaId, docIdOrTitle }) => {
    ...
    const idea = ideas.find((i: any) => i.id === ideaId);
    if (!idea) throw new Error(`Idea not found: ${ideaId}`);
    const doc = (idea.docs || []).find((d: any) =>
      d.id === docIdOrTitle || (d.title || "").toLowerCase().includes(docIdOrTitle.toLowerCase())
    );
    if (!doc) throw new Error(`Doc not found: ${docIdOrTitle}`);
    return { content: [{ type: "text", text: JSON.stringify(doc, null, 2) }] };
  }
);

server.tool(
  "vega_create_idea_doc",
  "Create a new document inside an idea (markdown, canvas, or diagram template).",
  {
    ideaId:   z.string(),
...
server.tool(
  "vega_update_idea_doc",
  "Update the title or content of a document inside an idea.",
  {
    ideaId:       z.string(),
  ...
  async ({ ideaId, docIdOrTitle, ...updates }) => {
    ...
    const docIdx = docs.findIndex((d: any) =>
      d.id === docIdOrTitle || (d.title || "").toLowerCase().includes(docIdOrTitle.toLowerCase())
    );
    if (docIdx === -1) throw new Error(`Doc not found: ${docIdOrTitle}`);
...
server.tool(
  "vega_delete_idea_doc",
  "Delete a document from an idea.",
  {
    ideaId:       z.string(),
    docIdOrTitle: z.string(),
  },
  async ({ ideaId, docIdOrTitle }) => {
    ...
    ideas[ideaIdx] = {
      ...ideas[ideaIdx],
      docs: (ideas[ideaIdx].docs || []).filter((d: any) =>
        d.id !== docIdOrTitle && !(d.title || "").toLowerCase().includes(docIdOrTitle.toLowerCase())
      ),
      updatedAt: now(),
    };
    const after = ideas[ideaIdx].docs.length;
    if (before === after) throw new Error(`Doc not found: ${docIdOrTitle}`);
    await vegaPost("/api/ideas", ideas);
